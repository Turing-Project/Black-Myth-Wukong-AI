...
class OpenAIProvider(LLMProvider, EmbeddingProvider):
    ...
    def _parse_config(self, provider_cfg) -> dict:
        ...
        if conf_dict[PROVIDER_SETTING_IS_AZURE]:
            ...
            key = api_keys[0]
            print("随机选择key为：",key)
            self.client = OpenAI(api_key=key,base_url="baseurl")
    ...
    def _emb_invocation_params(self) -> Dict:
        ...
        if self.provider_cfg[PROVIDER_SETTING_IS_AZURE]:
            engine = self._get_azure_deployment_id_for_model(
    ...
    def assemble_prompt_tripartite(self, template_str: str = None, params: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        # the system content defaults to the first paragraph of the template
        ...
        user_messages_part1_paragraphs = filtered_paragraphs[1:
                                                             image_introduction_paragraph_index]
        ...
        for paragraph in user_messages_part1_paragraphs:
            ...
            if not placeholder:
                ...
                placeholder_name = placeholder.replace(
                    "<$", "").replace("$>", "")
                ...
                if paragraph_input is None or paragraph_input == "" or paragraph_input == []:
                    ...
                    if isinstance(paragraph_input, str):
                        paragraph_content = paragraph.replace(
                            placeholder, paragraph_input)
                        user_messages_part1_contents.append(paragraph_content)
                    elif isinstance(paragraph_input, list):
                        paragraph_content = paragraph.replace(
                            placeholder, json.dumps(paragraph_input))
                        user_messages_part1_contents.append(paragraph_content)
                    else:
                        raise ValueError(
                            f"Unexpected input type: {type(paragraph_input)}")
        ...
        paragraph_input = params.get(
            constants.IMAGES_INPUT_TAG_NAME, [])  # 'image_introduction'

        if paragraph_input is None or paragraph_input == "" or paragraph_input == []:
            image_introduction_messages = []
        else:
            paragraph_content_pre = image_introduction_paragraph.replace(
            ...
            for item in paragraph_input:
                ...
                resolution = item.get(
                    constants.IMAGE_RESOLUTION_TAG_NAME, None)
            ...
            if not placeholder:
                ...
                if paragraph_input is None or paragraph_input == "" or paragraph_input == []:
                    ...
                    if isinstance(paragraph_input, str):
                        ...
                        paragraph_content = paragraph.replace(
                            placeholder, str(paragraph_input))
                        user_messages_part2_contents.append(paragraph_content)
                    elif isinstance(paragraph_input, list):
                        paragraph_content = paragraph.replace(
                            placeholder, json.dumps(paragraph_input))
